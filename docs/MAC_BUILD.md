...
Requirements
- macOS 12+ (or newer supported by UE5.7.1)
...
- Ensure that any FFmpeg dylibs that your app needs are either installed system-wide or copied into your app bundle (Contents/Frameworks) when packaging.
  - Optionally, when building with `-DFFMPEG_ROOT=/path/to/ffmpeg`, the build system will copy any `*.dylib` files found under `${FFMPEG_ROOT}/lib` into the app bundle `Contents/MacOS/Frameworks` and include them in the package. This is a convenience for reproducible test environments, but be mindful of licensing and distribution rules for FFmpeg.